...
- **Android preview APK** — ✅ shipped 2026-04-25.
  Build id `f6ce2d8c-10ab-41dc-a4a8-a602ef296593`, commit `b795e504`.
  Artifact: `https://expo.dev/artifacts/eas/wtDdCVw1WzqfSS5BiAvgSt.apk`.
  Cloud build wallclock: ~12 min. Keystore auto-generated by EAS.
- **iOS preview IPA** — ⏳ blocked on interactive Apple credentials
  setup. Run `eas credentials` (interactive) once to register an
  Apple Team + distribution certificate + provisioning profile for
  `com.omnibazaar.mobile`. Subsequent `npm run build:preview:ios`
  invocations are non-interactive.
